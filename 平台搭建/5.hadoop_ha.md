...
4 active节点执行格式化journal节点hdfs namenode -initializeSharedEdits
5 active节点执行hadoop-daemon.sh start namenode
6 standby 节点执行hdfs namenode -bootstrapStandby
7 active节点执行hadoop-daemon.sh stop namenode
8 active节点执行hdfs zkfc -formatZK
9 active节点执行start-dfs.sh
```
# 注
```
